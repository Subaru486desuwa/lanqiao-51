/*头文件声明区*/
#include <STC15F2K60S2.H>
#include <Key.h>
#include <Seg.h>
#include <iic.h>
#include <LED.h>
#include <Uart.h>
#include <stdio.h>
#include <onewire.h>

/*全局变量声明区域*/
unsigned char Key_Val,Key_Down,Key_Old; //按键消抖专用变量
unsigned char Seg_Buf[8]={10,10,10,10,10,10,10,10};  //数码管段码数据存储数组
unsigned char Point[8]={0,0,0,0,0,0,0,0}; //数码管小数点数据存储数组
unsigned char Seg_Pos;//数码管位选

unsigned char ucLED[8]={0,0,0,0,0,0,0,0}; //LED使能数据存储数组

unsigned int Seg_Slow_Down;//数码管减速专用变量-300ms
unsigned char Key_Slow_Down;//按键减速专用变量-10ms
unsigned char Uart_Slow_Down;//串口减速专用变量-200ms

unsigned char Uart_Recv_Index=0;	//串口接收数组指针
unsigned char Uart_Recv[10];		//串口接收数据数组 默认长度10

bit Seg_Dis_Mode;	//0-温度 1-电压

float temperature;//读取ds18b20电压
float Voltage;//rb2电压

bit LED_Flag;
bit Lock_Flag;	//0-解锁 1-上锁
unsigned char Timer_100Ms;//0.1s

bit Relay_Flag;
bit Buzz_Flag;

/*按键处理函数*/
void Key_Proc()
{
  if(Key_Slow_Down) return;
  Key_Slow_Down=1;							 //按键减速

  Key_Val=Key_Read();
  Key_Down=Key_Val&(Key_Val^Key_Old);
  Key_Old=Key_Val;						  //按键消抖

	switch(Key_Down)
	{
		case 4:
			if(Lock_Flag==0)
				Lock_Flag=1;
		break;
		
		case 5:
			if(Lock_Flag==1)
				Lock_Flag=0;
		break;
			
		case 12:
			if(Seg_Dis_Mode==0)
				printf("TEMP:%.1f\r\n",temperature);
			else
				printf("Voltage:%.2f\r\n",Voltage);

		break;
	}
}

/*数码管处理函数*/
void Seg_Proc()
{
  if(Seg_Slow_Down) return;
  Seg_Slow_Down=1;
	
	temperature=Read_Temperature();
	Ad_Read(0x43);
	Voltage=Ad_Read(0x43)/51.0;
	
	if(Seg_Dis_Mode==0)//温度显示
	{
		Seg_Buf[0]=11;//U
		Seg_Buf[1]=1;
		Seg_Buf[5]=(unsigned char)temperature/10%10;
		Seg_Buf[6]=(unsigned char)temperature%10;
		Seg_Buf[7]=(unsigned int)(temperature*10)%10;
		Point[6]=1;
		Point[5]=0;
	}
	else
	{
		Seg_Buf[0]=11;
		Seg_Buf[1]=2;
		Seg_Buf[5]=(unsigned char)Voltage;
		Seg_Buf[6]=(unsigned int)(Voltage*100)/10%10;
		Seg_Buf[7]=(unsigned int)(Voltage*100)/100%10;
		Point[6]=0;
		Point[5]=1;
	}

}

/*LED处理函数*/
void LED_Proc()
{
	ucLED[0]=Seg_Dis_Mode==0;
	ucLED[1]=Seg_Dis_Mode==1;
	ucLED[2]=Lock_Flag && LED_Flag;
	

	Relay_Flag=(temperature>=28)?0:1;
	Buzz_Flag=(Voltage>3.6)?0:1;
	
	Buzz(Buzz_Flag);
	Relay(Relay_Flag);
}

/*串口处理函数*/
void Uart_Proc()
{
	if(Uart_Slow_Down) return;
	Uart_Slow_Down=1;
	
	if(Lock_Flag==0)	//解锁
	{
		if(Uart_Recv_Index==1)	//长度达标
		{
			if(Uart_Recv[0]=='A')
			{
				Seg_Dis_Mode=0;
				Uart_Recv_Index=0;
			}
			if(Uart_Recv[0]=='B')
			{
				Seg_Dis_Mode=1;
				Uart_Recv_Index=0;
			}

		}	
	}
}


/*串口1中断服务函数*/
void Uart1Server() interrupt 4
{
	if(RI== 1 && Uart_Recv_Index <8)	//串口接收数据	限制
	{
		Uart_Recv[Uart_Recv_Index] = SBUF;
		Uart_Recv_Index++; 		//处理函数中，用完的接收指针需要归零
		RI=0;
	}
}

/*定时器0中断初始化函数*/
void Timer0Init(void)		//1毫秒@12.000MHz
{
	AUXR &= 0x7F;			//定时器时钟12T模式
	TMOD &= 0xF0;			//设置定时器模式
	TL0 = 0x18;				//设置定时初始值
	TH0 = 0xFC;				//设置定时初始值
	TF0 = 0;				//清除TF0标志
	TR0 = 1;				//定时器0开始计时
	ET0=1;
	EA=1;
}



/*定时器0中断服务函数*/
void Timer0Server() interrupt 1
{
	if(++Key_Slow_Down==10)
		Key_Slow_Down=0;			//按键减速

	if(++Seg_Slow_Down==300)
		Seg_Slow_Down=0;			//数码管减速
	
	if(++Uart_Slow_Down==200)
		Uart_Slow_Down=0;			//串口减速
	
	if(++Timer_100Ms==100)
	{
		Timer_100Ms=0;
		LED_Flag^=1;
	}
	if(++Seg_Pos==8)
	{
	 Seg_Pos=0;				   //数码管位选
	}
	Seg_Dis(Seg_Pos,Seg_Buf[Seg_Pos],Point[Seg_Pos]);
	LED_Dis(Seg_Pos,ucLED[Seg_Pos]); 
}

void Delay750ms()		//@12.000MHz
{
	unsigned char i, j, k;

	i = 35;
	j = 51;
	k = 182;
	do
	{
		do
		{
			while (--k);
		} while (--j);
	} while (--i);
}


/*main*/
void main()
{
	Read_Temperature();
	Delay750ms();
	
  UartInit();
  Timer0Init();
  while(1)
  {
   Key_Proc();
   Seg_Proc();
   LED_Proc();
	 Uart_Proc();
  }
}


